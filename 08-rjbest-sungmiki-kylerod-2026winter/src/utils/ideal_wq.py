...
def ideal_wq(df, path='data/wq.csv'):
    ...
    df['DRP'] = [1.0 if _ < 100 else 0.0 for _ in df['dissolved reactive phosphorus']]
    df['EC'] = [1.0 if _ < 800 else 0.0 for _ in df['electrical conductance']]
    df['Alkalinity'] = [1.0 if _ > 20 and _ < 200 else 0.0 for _ in df['total alkalinity']]
    ...
    df['ideal'] = np.where(df['how_good'] > 2, 1.0, 0.0)
    df['unideal'] = np.where(df['how_good'] < 1, 1.0, 0.0)
